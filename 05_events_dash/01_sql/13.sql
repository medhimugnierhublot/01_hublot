...
wishlist_dedup AS (
  SELECT
    ...
    cm.campaign_id,
    cm.account_id,
    FORMAT('%s: %s', FORMAT_DATE('%Y-%m-%d', w.created_date), p.collection_c) AS entry
  FROM campaign_members cm
  JOIN `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_wishlist_c_filtered` w
    ON cm.account_id = w.account_c
  LEFT JOIN `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_product_2_filtered` p
    ON w.product_c = p.id
...
SELECT
  ...
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(b.purchase_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 30 THEN DATE(b.purchase_date) END) AS converted_1m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(b.purchase_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 90 THEN DATE(b.purchase_date) END) AS converted_3m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(b.purchase_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 180 THEN DATE(b.purchase_date) END) AS converted_6m,


  SUM(CASE WHEN DATE_DIFF(DATE(b.purchase_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 30 THEN b.retail_price_chf_c ELSE 0 END) AS revenue_1m,
  SUM(CASE WHEN DATE_DIFF(DATE(b.purchase_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 90 THEN b.retail_price_chf_c ELSE 0 END) AS revenue_3m,
  SUM(CASE WHEN DATE_DIFF(DATE(b.purchase_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 180 THEN b.retail_price_chf_c ELSE 0 END) AS revenue_6m,


  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(w.created_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 30 THEN DATE(w.created_date) END) AS wishes_1m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(w.created_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 90 THEN DATE(w.created_date) END) AS wishes_3m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(w.created_date), PARSE_DATE('%Y-%m-%d', cb.campaign_end_date), DAY) BETWEEN 0 AND 180 THEN DATE(w.created_date) END) AS wishes_6m,
...
LEFT JOIN (
  ...
  WHERE is_watch_c = TRUE
    AND DATE(COALESCE(warranty_activation_date_c, purchase_date_c)) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 180 DAY) AND CURRENT_DATE()
) b
  ON cm.account_id = b.account_c

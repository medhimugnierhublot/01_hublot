...
wishlist_aggregated AS (
  SELECT
    ...
    IFNULL(
      STRING_AGG(
        FORMAT('%s: %s', FORMAT_DATE('%Y-%m-%d', w.created_date), w.collection_c),
        '\n'
      ),
      ''
    ) AS wishlist_activity
  FROM wishlist w
  JOIN campaign_base cb ON DATE(w.created_date) BETWEEN DATE(cb.end_date) AND DATE_ADD(DATE(cb.end_date), INTERVAL 6 MONTH)
...
LEFT JOIN wishlist_aggregated wa ON w.account_c = wa.account_c AND cb.campaign_id = wa.campaign_id

...
SELECT
  ...
  -- Wishlist dictionary: (created_date, collection)
  ARRAY(
    SELECT STRUCT(DATE(w.created_date) AS created_date, w.collection_c AS collection)
    FROM UNNEST(ARRAY_AGG(
      CASE WHEN DATE_DIFF(DATE(w.created_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 180 
           THEN STRUCT(w.created_date, w.collection_c)
           ELSE NULL
      END IGNORE NULLS)) AS w
  ) AS wishlist_activity,


  -- Sales dictionary: (purchase_date, price)
  ARRAY(
    SELECT STRUCT(DATE(b.purchase_date) AS purchase_date, b.retail_price_chf_c AS price)
    FROM UNNEST(ARRAY_AGG(
      CASE WHEN DATE_DIFF(DATE(b.purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 180 
           THEN STRUCT(b.purchase_date, b.retail_price_chf_c)
           ELSE NULL
      END IGNORE NULLS)) AS b
  ) AS sales_details

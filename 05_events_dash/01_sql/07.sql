...
SELECT
  ...
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 30 THEN purchase_date END) AS converted_1m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 90 THEN purchase_date END) AS converted_3m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 180 THEN purchase_date END) AS converted_6m,


  SUM(CASE WHEN DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 30 THEN retail_price_chf_c ELSE 0 END) AS revenue_1m,
  SUM(CASE WHEN DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 90 THEN retail_price_chf_c ELSE 0 END) AS revenue_3m,
  SUM(CASE WHEN DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 180 THEN retail_price_chf_c ELSE 0 END) AS revenue_6m,


  SUM(CASE WHEN cm.participated_c = TRUE AND DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 30 THEN 1 ELSE 0 END) AS converted_1m_has_participated,
  SUM(CASE WHEN cm.participated_c = TRUE AND DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 90 THEN 1 ELSE 0 END) AS converted_3m_has_participated,
  SUM(CASE WHEN cm.participated_c = TRUE AND DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 180 THEN 1 ELSE 0 END) AS converted_6m_has_participated,


  SUM(CASE WHEN cm.participated_c = TRUE AND DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 30 THEN retail_price_chf_c ELSE 0 END) AS revenue_1m_has_participated,
  SUM(CASE WHEN cm.participated_c = TRUE AND DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 90 THEN retail_price_chf_c ELSE 0 END) AS revenue_3m_has_participated,
  SUM(CASE WHEN cm.participated_c = TRUE AND DATE_DIFF(DATE(purchase_date), DATE(cb.end_date), DAY) BETWEEN 0 AND 180 THEN retail_price_chf_c ELSE 0 END) AS revenue_6m_has_participated,
  ...
  (
    SELECT IFNULL(
      STRING_AGG(
        FORMAT('%s: CHF%.0f', FORMAT_DATE('%Y-%m-%d', COALESCE(warranty_activation_date_c, purchase_date_c)), retail_price_chf_c),
        '\n'
      ),
      ''
    )
    FROM (
      ...
      WHERE account_c = cm.account_id
        AND is_watch_c = TRUE
        AND DATE(COALESCE(warranty_activation_date_c, purchase_date_c)) BETWEEN DATE(cb.end_date) AND DATE_ADD(DATE(cb.end_date), INTERVAL 6 MONTH)
    )
  ) AS sales_details

...
SELECT
  ...
  -- ✅ Deduplicated sales_details
  (
    SELECT STRING_AGG(
      FORMAT('%s: %s (CHF%.0f)', FORMAT_DATE('%Y-%m-%d', COALESCE(warranty_activation_date_c, purchase_date_c)), INITCAP(product_reference_c), retail_price_chf_c),
      '\n'
    )
    FROM (
      SELECT DISTINCT
      ...
      WHERE account_c = cm.account_id
        ...
        AND DATE(COALESCE(warranty_activation_date_c, purchase_date_c))
            BETWEEN DATE(cb.subcampaign_end_date) AND DATE_ADD(DATE(cb.subcampaign_end_date), INTERVAL 3 MONTH)
    )
  ) AS sales_details

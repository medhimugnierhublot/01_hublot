...
campaign_members AS (
  ...
  -- and 
  -- ap.id = "0010X00004veSMlQAM" and 
  -- cm.campaign_id = '701J5000000PhynIAC'
),
...
sales_aggregated AS (
  SELECT
    ...
    -- -- sales_details: list of product and date
    -- (
    --   SELECT STRING_AGG(DISTINCT FORMAT('%s: %s (CHF%.0f)', FORMAT_DATE('%Y-%m-%d', purchase_date), INITCAP(product_reference_c), retail_price_chf_c), '\n')
    --   FROM UNNEST(ARRAY_AGG(STRUCT(purchase_date, product_reference_c, retail_price_chf_c))) AS s
    -- ) AS sales_details

...
-- Pre-aggregate wishlist activity
wishlist_aggregated AS (
  SELECT
    w.account_c,
    cb.campaign_id,
    STRING_AGG(
    ...
    FROM (SELECT DISTINCT account_c, created_date, collection_c FROM wishlist) w -- Deduplicate wishlist entries
  JOIN campaign_base cb
    ON w.account_c = cb.campaign_or_subcampaign_id  -- correcting the join condition
   AND DATE(w.created_date) BETWEEN DATE(cb.end_date) AND DATE_ADD(DATE(cb.end_date), INTERVAL 6 MONTH)
  GROUP BY w.account_c, cb.campaign_id
),
...
SELECT
  ...
  COALESCE(wa.wishlist_activity, '') AS wishlist_activity,

  -- Deduplicated sales_details per member + campaign
  (
    SELECT IFNULL(
      STRING_AGG(
        FORMAT('%s: CHF%.0f', FORMAT_DATE('%Y-%m-%d', purchase_date), retail_price_chf_c),
        '\n'
      ),
      ''
    )
...
LEFT JOIN wishlist_aggregated wa ON cm.account_id = wa.account_c AND cb.campaign_id = wa.campaign_id

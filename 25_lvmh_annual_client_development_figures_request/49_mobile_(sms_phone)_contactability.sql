...
regional_rollup AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN person_mobile_phone IS NOT NULL THEN account_id END) AS clients_with_mobile
  FROM region_mapping
  GROUP BY region
),
...
global_rollup AS (
  SELECT
    ...
    SUM(clients_with_mobile) AS clients_with_mobile
  FROM regional_rollup
)

SELECT
  SAFE_DIVIDE(SUM(CASE WHEN region='Global' THEN clients_with_mobile END),
              SUM(CASE WHEN region='Global' THEN total_clients END)) AS Global_Figures_Input,
  SAFE_DIVIDE(SUM(CASE WHEN region='Europe' THEN clients_with_mobile END),
              SUM(CASE WHEN region='Europe' THEN total_clients END)) AS Europe,
  SAFE_DIVIDE(SUM(CASE WHEN region='North America' THEN clients_with_mobile END),
              SUM(CASE WHEN region='North America' THEN total_clients END)) AS North_America,
  SAFE_DIVIDE(SUM(CASE WHEN region='Middle East' THEN clients_with_mobile END),
              SUM(CASE WHEN region='Middle East' THEN total_clients END)) AS Middle_East,
  SAFE_DIVIDE(SUM(CASE WHEN region='APAC' THEN clients_with_mobile END),
              SUM(CASE WHEN region='APAC' THEN total_clients END)) AS APAC,
  SAFE_DIVIDE(SUM(CASE WHEN region='China' THEN clients_with_mobile END),
              SUM(CASE WHEN region='China' THEN total_clients END)) AS China,
  SAFE_DIVIDE(SUM(CASE WHEN region='Japan' THEN clients_with_mobile END),
              SUM(CASE WHEN region='Japan' THEN total_clients END)) AS Japan,
  SAFE_DIVIDE(SUM(CASE WHEN region='Korea' THEN clients_with_mobile END),

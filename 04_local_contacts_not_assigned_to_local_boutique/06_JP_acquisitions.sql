...
SELECT
    ...
    bout.billing_country AS boutique_country,
...
WHERE
    ...
    AND acc.billing_country_code = 'JP'
    AND acc.billing_country_code <> bout.billing_country_code
    AND (
        -- For non-Prospects, apply last purchase filter
        (acc.macro_segment NOT IN ('Prospect','Loyal','VIC','VVIC') AND CAST(COALESCE(acc.last_dos_purchase_date_c, bs.last_purchase_date) AS DATE) < DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH))

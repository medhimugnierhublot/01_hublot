...
WHERE
    ...
    AND bout.billing_country_code = 'CH'
    ...
    AND NOT (
        acc.billing_country_code = 'FR'
        AND bout.name IN ('Hublot Geneva Boutique', 'Hublot Monaco Boutique')
    )
    AND (
        -- Clients: must not have purchased in 12 months
        ...
        -- Prospects: only if older than 6 months

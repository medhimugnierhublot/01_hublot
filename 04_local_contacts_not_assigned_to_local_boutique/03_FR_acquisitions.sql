...
WHERE
    ...
    AND acc.billing_country_code = 'FR'
    AND bout.name NOT IN ('Hublot Geneva Boutique', 'Hublot Monaco Boutique')
    AND acc.billing_country_code <> bout.billing_country_code
    AND (
        -- For non-Prospects, apply last purchase filter
        (acc.macro_segment NOT IN ('Prospect','Loyal','VIC','VVIC') AND 
        CAST(COALESCE(acc.last_dos_purchase_date_c, bs.last_purchase_date) AS DATE) < DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH))
        
        -- For Prospects, allow only if older than 6 months
        OR (acc.macro_segment = 'Prospect' AND CAST(acc.created_date AS DATE) < DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH))
    );
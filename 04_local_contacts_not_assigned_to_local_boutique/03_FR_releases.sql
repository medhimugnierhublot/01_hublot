...
latest_salesperson AS (
  ...
  FROM (
    SELECT
      account_id,
      sales_person_new_c,
      ROW_NUMBER() OVER (PARTITION BY account_id ORDER BY photo_date DESC) AS rn
    FROM `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_account_history_dataset`
    WHERE sales_person_new_c IS NOT NULL
  ) ah
...
LEFT JOIN latest_salesperson sp
    ON acc.id = sp.account_id


WHERE
    acc.permission_to_contact_pc = TRUE
    AND COALESCE(task_count.nb_offline_actions, 0) < 2
    AND bout.billing_country_code = 'FR'

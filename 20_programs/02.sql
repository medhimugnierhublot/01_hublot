...
SELECT
  ...
  -- Outreaches
  COUNT(DISTINCT CASE WHEN DATE_DIFF(o.created_date, cl.created_date, DAY) BETWEEN 0 AND 30 THEN o.created_date END) AS outreaches_1m,
  COUNT(DISTINCT CASE WHEN DATE_DIFF(o.created_date, cl.created_date, DAY) BETWEEN 0 AND 90 THEN o.created_date END) AS outreaches_3m,
  ...
  --     SELECT DISTINCT
  --       w2.created_date,
  --       w2.collection_c AS collection
  --     FROM wishlist w2
  --     WHERE w2.account_c = clm.account_id
  --       AND DATE_DIFF(w2.created_date, cl.created_date, DAY) BETWEEN 0 AND 30
  --   )
  -- ) AS wishes_1m_details,
  ...
  --     FROM wishlist w2
  --     WHERE w2.account_c = clm.account_id
  --       AND DATE_DIFF(w2.created_date, cl.created_date, DAY) BETWEEN 0 AND 90
  --   )
  -- ) AS wishes_3m_details,
  ...
  --       retail_price_chf_c
  --     FROM belonging
  --     WHERE account_c = clm.account_id
  --       AND DATE_DIFF(purchase_date, cl.created_date, DAY) BETWEEN 0 AND 30
  --   )
  -- ) AS sales_1m_details,
  ...
  --     FROM belonging
  --     WHERE account_c = clm.account_id
  --       AND DATE_DIFF(purchase_date, cl.created_date, DAY) BETWEEN 0 AND 90
  --   )
  -- ) AS sales_3m_details
...
LEFT JOIN outreaches o ON o.account_id = clm.account_id
LEFT JOIN wishlist w ON w.account_c = clm.account_id
LEFT JOIN belonging b ON b.account_c = clm.account_id

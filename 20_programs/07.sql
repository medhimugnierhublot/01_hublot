...
SELECT
  ...
  (
    ...
    FROM (
      ...
      LEFT JOIN `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_product_2_filtered` prd
        ON bel.product_code_c = prd.product_code
      WHERE bel.account_c = clm.account_id
        AND bel.is_watch_c = TRUE
        AND DATE(COALESCE(bel.warranty_activation_date_c, bel.purchase_date_c))
          BETWEEN DATE(lb.created_date)
          AND DATE_ADD(DATE(lb.created_date), INTERVAL 1 MONTH)
    )
  ) AS sales_details_1m,
  ...
  (
    ...
    FROM (
      ...
      WHERE bel.account_c = clm.account_id
        ...
        AND DATE(COALESCE(bel.warranty_activation_date_c, bel.purchase_date_c))
          BETWEEN DATE(lb.created_date)
          AND DATE_ADD(DATE(lb.created_date), INTERVAL 3 MONTH)
    )
  ) AS sales_details_3m,
  ...
  (
    ...
    WHERE wish.account_c = clm.account_id
      AND DATE(wish.created_date) BETWEEN DATE(lb.created_date)
      AND DATE_ADD(DATE(lb.created_date), INTERVAL 1 MONTH)

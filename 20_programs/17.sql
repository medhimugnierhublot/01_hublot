-- Track upgrades from 'One Timer' to higher segments (Loyal, VIC, VVIC), including inferred 3-month upgrades
...
segment_ranked AS (
  SELECT 'Prospect' AS segment, 1 UNION ALL
  SELECT 'One Timer', 2 UNION ALL
  SELECT 'Loyal', 3 UNION ALL
  SELECT 'VIC', 4 UNION ALL
  SELECT 'VVIC', 5
),
...
SELECT
  ...
  (
    ...
    -- Offline action details 1m
    (
    SELECT STRING_AGG(
        FORMAT('%s (Offline Action)', FORMAT_DATE('%Y-%m-%d', tsks.activity_date)),
        '\n'
    )
    FROM offline_actions_attributed tsks
    WHERE tsks.account_id = clm.account_id
        AND DATE(tsks.activity_date) BETWEEN DATE(cb.created_date) AND DATE_ADD(DATE(cb.created_date), INTERVAL 1 MONTH)
    ) AS offline_actions_details_1m,


    -- Offline action details 3m
    (
    SELECT STRING_AGG(
        FORMAT('%s (Offline Action)', FORMAT_DATE('%Y-%m-%d', tsks.activity_date)),
        '\n'
    ...
    FROM offline_actions_attributed tsks
    WHERE tsks.account_id = clm.account_id
        AND DATE(tsks.activity_date) BETWEEN DATE(cb.created_date) AND DATE_ADD(DATE(cb.created_date), INTERVAL 3 MONTH)
  ...
  IF(
    ...
    FORMAT('one_timer_to_%s', LOWER(REPLACE(ud.segment_1m, ' ', '_'))),
    NULL
  ) AS upgraded_1m_details,


  -- Upgrade in 3m if upgraded in 1m OR 3m (allow inferred upgrade if snapshot missing)
  IF(
    (ud.segment_1m IS NOT NULL AND ud.segment_1m != 'One Timer')
    OR (ud.segment_3m IS NOT NULL AND ud.segment_3m != 'One Timer'),
    1,
    0
  ) AS upgraded_3m,
  ...
  IF(
    ...
    FORMAT('one_timer_to_%s', LOWER(REPLACE(ud.segment_3m, ' ', '_'))),
    IF(
      ud.segment_1m IS NOT NULL AND ud.segment_1m != 'One Timer',
      FORMAT('one_timer_to_%s', LOWER(REPLACE(ud.segment_1m, ' ', '_'))),
      NULL

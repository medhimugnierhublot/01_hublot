...
offline_actions_attributed AS (
  ...
  FROM client_list_members cm
  LEFT JOIN list_base cb ON cb.list_id = cm.client_list_c
  LEFT JOIN `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_task_filtered` tsks
    ON cm.account_id = tsks.account_id
  ...
  LEFT JOIN `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_user_role_filtered` role
    ON user.user_role_id = role.id
  WHERE
    tsks.channel_c = 'Action'
    AND tsks.type_c IS NOT NULL
    AND DATE(tsks.activity_date) BETWEEN DATE(cb.created_date) AND DATE_ADD(DATE(cb.created_date), INTERVAL 90 DAY)
    -- AND (

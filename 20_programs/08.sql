...
SELECT
  ...
  (
    ...
    WHERE sa.client_c = cm.member_id AND sa.list_id = cb.list_id
  ) AS sales_details,
  ...
  (
    ...
    WHERE wish.account_c = clm.account_id
      AND DATE(wish.created_date) BETWEEN DATE(lb.created_date) AND DATE_ADD(DATE(lb.created_date), INTERVAL 1 MONTH)
  ...
  (
    ...
    LEFT JOIN `hub-prd-atomdt-prj-pltfrm-12b.hub_prd_atomdtp_bqd_crm_client_analysis.CLIENTS_REPORTING_product_2_filtered` prd
      ON wish.product_c = prd.id
    WHERE wish.account_c = clm.account_id
      AND DATE(wish.created_date) BETWEEN DATE(lb.created_date) AND DATE_ADD(DATE(lb.created_date), INTERVAL 3 MONTH)
  ...
  (
    ...
    WHERE tsks.account_id = clm.account_id
      AND DATE(tsks.created_date) BETWEEN DATE(lb.created_date) AND DATE_ADD(DATE(lb.created_date), INTERVAL 1 MONTH)
  ...
  (
    ...
    FROM outreaches_attributed tsks
    WHERE tsks.account_id = clm.account_id
      AND DATE(tsks.created_date) BETWEEN DATE(lb.created_date) AND DATE_ADD(DATE(lb.created_date), INTERVAL 3 MONTH)
...
LEFT JOIN outreaches_aggregated oa ON oa.client_c = clm.client_c AND oa.list_id = lb.list_id
